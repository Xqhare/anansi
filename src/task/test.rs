...
fn equality() {
    ...
    let task2 = Task::new(" (A) test");
    assert!(task1 == task2);
    let task3 = Task::new("(B) test");
    assert!(task1 != task3);
}
...
#[test]
fn context_tags() {
    let task1 = Task::new("(A) test @context @context2 @test @test2 @test3 key1:value1 key2:value2"); 
    assert_eq!(task1.contexts(), &vec!["context".to_string(), "context2".to_string(), "test".to_string(), "test2".to_string(), "test3".to_string()]);
    let task2 = Task::new("(A) test +project @context1 @context2 key1:value1 key2:value2");
    assert_eq!(task2.contexts().len(), 2);
    let task3 = Task::new("(A) test +project key1:value1 key2:value2 web@email.com @ context1 @context2");
    assert_eq!(task3.contexts().len(), 1);
}

#[test]
fn project_tags() {
    let task1 = Task::new("(A) test +project +project2 +test +test2 +test3 key1:value1 key2:value2"); 
    assert_eq!(task1.projects(), &vec!["project".to_string(), "project2".to_string(), "test".to_string(), "test2".to_string(), "test3".to_string()]);
    let task2 = Task::new("(A) test +project +project2 +test +test2 +test3 @context1 @context2 key1:value1 key2:value2");
    assert_eq!(task2.projects().len(), 5);
    let task3 = Task::new("(A) test +project 1 + 2, 1+2 key1:value1 key2:value2 web@email.com @ context1 @context2");
...
fn special_tags() {
    ...
    let task3 = Task::new("(A) test +project test: hello! :2 : testing::test 1 + 2, 1+2 key1:value1 key2:value2 web@email.com @ context1 @context2");
...
fn dates() {
    ...
    let task2 = Task::new("(A) 2010-10-20 2010-10-01 test +project +project2 +test +test2 +test3 key1:value1 key2:value2");
    assert_eq!(task2.completion_date(), "2010-10-20");
    assert_eq!(task2.inception_date(), "2010-10-01");

    let mut task3 = Task::new("(A) 2020-12-31 test +project +project2 +test +test2 +test3 key1:value1 key2:value2");
    assert_eq!(task3.completion_date(), "");
...
fn priorities() {
    ...
    for letter in tmp {
        let task = Task::new(format!("({}) test +project +project2 +test +test2 +test3 key1:value1 key2:value2", letter));
